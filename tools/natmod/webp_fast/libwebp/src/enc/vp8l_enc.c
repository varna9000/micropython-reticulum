...
static int ApplyPalette(const uint32_t* src, uint32_t src_stride, uint32_t* dst,
  ...
  if (palette_size < APPLY_PALETTE_GREEDY_MAX) {
    ...
    /* natmod: 'static' so this const function-pointer table lands in
       relocated-rodata (GOT) rather than a writable .data copy mpy_ld rejects. */
    static uint32_t (*const hash_functions[])(uint32_t) = {
