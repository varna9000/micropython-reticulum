...
/* natmod: const so this function-pointer table lands in relocated-rodata
   (GOT) instead of writable .data.rel that mpy_ld rejects. The worker
   interface is never swapped in this build (see WebPSetWorkerInterface). */
static const WebPWorkerInterface g_worker_interface = {
...
int WebPSetWorkerInterface(const WebPWorkerInterface* const winterface) {
  ...
  (void)winterface;  /* natmod: worker interface is fixed; not swappable */
  return 1;
}
