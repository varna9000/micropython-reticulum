...
// natmod patch: original initializes a static pointer to its own address
// (a sentinel), which emits a self-relocating .data.rel.local that mpy_ld
// can't link. A plain int flag is functionally identical (one-time init)
// and lives in .bss (zero-init, no relocation).
#define WEBP_DSP_INIT(func)                  \
  do {                                       \
    static int func##_last_cpuinfo_used = 0; \
